...
const HeroCard = ({ title, description, icon }) => {
    ...
    return (
        <>
            <div className={` !flex !w-full !justify-center ${path === '/' ? '' : '!flex !w-full !justify-center'}`}>
                <div
                    className={`rounded-[20px] flex justify-center items-center transition-all duration-300 flex-col min-h-[270px] relative p-2 shadow-[0px_8px_16px_0px_#00000029] ${path === '/' ? 'max-md:min-h-[170px] max-w-[330px] bg-light-blue w-full ease-linear hover:shadow-[0px_5px_30px_-5px_#fff]' : 'bg-white h-full max-sm:h-[275px] hover:shadow-[0px_5px_30px_-5px_#000] w-full max-sm:!max-w-[340px]'}`}
                >
                    <Image
                    ...
                    <div className="xl:p-7 lg:p-3 p-2 flex justify-center items-center flex-col">
                        ...
                        <h4 className={`text-26 font-bold leading-131 text-center mx-auto max-w-[250px] ${path === '/' ? 'text-white max-sm:font-medium max-lg:text-2xl max-md:text-xl max-sm:text-base mt-6 max-md:mt-5 max-sm:mt-4' : 'text-dark-blue italic !max-w-[226px]  mt-2'}`}>
                            {title}
                        </h4>
